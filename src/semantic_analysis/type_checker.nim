...
proc areTypesCompatible(checker: TypeChecker, expected: Type, actual: Type, 
                       pos: Position, isVarArgs = false): bool =
  ...
  of NkFunctionCall:
    ...
    if callee.kind == NkIdentifier:
      ...
      if symbolOpt.isNone:
        ...
        let hasVarArgs = paramTypes.len > 0 and 
                         paramTypes[^1].kind == TkMeta and 
        ...
        let inferencer = newTypeInferencer(checker.fileInfo)
        let regularParamCount = if hasVarArgs: paramTypes.len - 1 else: paramTypes.len

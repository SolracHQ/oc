...
proc extractLine(source: string, position: int): tuple[line: string, columnOffset: int] =
...
proc logError*(stage: string, error: string, position: Position, source: string) =
  ...
  stdout.styledWriteLine(fgCyan, position.file.path & ":" & $position.line & ":" & $position.column)
